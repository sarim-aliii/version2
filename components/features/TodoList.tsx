...
export const TodoList: React.FC = () => {
    const { activeProject, updateActiveProjectData } = useAppContext();
    const [todos, setTodos] = useState<Todo[]>(activeProject?.todos || []);
    ...
    const saveTodos = (newTodos: Todo[]) => {
        setTodos(newTodos);
        if (activeProject) {
            // This calls the API to persist data
            updateActiveProjectData({ todos: newTodos });
        }
    };
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                <input
                    ...
                    className="flex-grow bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md p-2 text-sm text-slate-800 dark:text-slate-300 focus:ring-1 focus:ring-red-500 focus:outline-none transition-colors"
                />
                <select 
                    value={priority}
                    onChange={(e) => setPriority(e.target.value as Priority)}
                    className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md p-2 text-sm text-slate-800 dark:text-slate-300 focus:ring-1 focus:ring-red-500 focus:outline-none transition-colors"
                >
